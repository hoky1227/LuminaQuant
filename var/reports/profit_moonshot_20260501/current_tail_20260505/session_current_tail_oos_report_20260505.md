...
- **User-gate pass / deployment-review candidate:** `profit_moonshot_leadlag_slow_diffusion_mode`.
  ...
  - It should supersede `profit_moonshot_momentum_hybrid_safe_mode` for the next deployment review, but it still deserves cautious shadow sizing because OOS MDD is `7.0817%` and OOS Sharpe is small.
...
- `cross_crypto_slow_diffusion`: **2 screen survivors**. The live-equivalent candidate above is already the top survivor's implemented alpha family; no additional full backtest was launched in this phase.
...
- `funding_taker_flow` family: rejected before full backtest; cheap raw-first screen produced zero survivors after costs and OOS checks.
...
3. Next candidate must beat the refreshed lead-lag bar: complete raw-first train/val/OOS, positive OOS return, positive OOS Sharpe/Sortino, zero liquidations, and RSS under 8GB.
