...
def recommend_operating_mode(
    *,
    ...
    rationale: list[str] = [
        ...
        f"hybrid_health healthy={bool(hybrid.get('healthy'))} stage={hybrid.get('recommended_stage') or 'n/a'!s} oos_return={_safe_float(hybrid.get('oos_total_return'), 0.0):+.4%} oos_sharpe={_safe_float(hybrid.get('oos_sharpe'), 0.0):.4f}",
    ]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--three-way-allocator-path", type=Path, default=DEFAULT_THREE_WAY_ALLOCATOR_PATH)
    parser.add_argument("--operating-plan-path", type=Path, default=DEFAULT_OPERATING_PLAN_PATH)
    parser.add_argument("--balanced-strategy-path", type=Path, default=DEFAULT_BALANCED_STRATEGY_PATH)
    parser.add_argument("--hybrid-portfolio-path", type=Path, default=DEFAULT_HYBRID_PORTFOLIO_PATH)
    parser.add_argument("--materialized-root", type=Path, default=DEFAULT_MATERIALIZED_ROOT)
    parser.add_argument("--raw-aggtrades-root", type=Path, default=DEFAULT_RAW_AGGTRADES_ROOT)
...
def main() -> None:
    ...
    market_judgement = _read_json(Path(args.market_judgement_path).resolve())
    market_judgement["_path"] = str(Path(args.market_judgement_path).resolve())
    soft_allocator = _read_json(Path(args.soft_allocator_path).resolve())
    soft_allocator["_path"] = str(Path(args.soft_allocator_path).resolve())
    three_way_allocator = _read_json(Path(args.three_way_allocator_path).resolve())
    three_way_allocator["_path"] = str(Path(args.three_way_allocator_path).resolve())
    operating_plan = _read_json(Path(args.operating_plan_path).resolve())
    operating_plan["_path"] = str(Path(args.operating_plan_path).resolve())
    balanced_strategy_path = Path(args.balanced_strategy_path).resolve()
    balanced_strategy = _read_json(balanced_strategy_path) if balanced_strategy_path.exists() else None
    hybrid_portfolio_path = Path(args.hybrid_portfolio_path).resolve()
    ...
    as_of_date = _parse_as_of_date(dict(latest_live_judgement).get("date"))
    pair_symbols = list(args.pair_symbols) or list(DEFAULT_PAIR_SYMBOLS)
    volume_signals = [
        _load_symbol_volume_signal(
            raw_aggtrades_root=Path(args.raw_aggtrades_root).resolve(),
            symbol=symbol,
