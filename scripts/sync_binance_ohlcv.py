...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--timeframe",
        default=BaseConfig.TIMEFRAME,
        help="OHLCV timeframe token (e.g., 1m, 5m, 1h, 1d).",
    )
    parser.add_argument(
        "--db-path",
        default=getattr(BaseConfig, "MARKET_DATA_SQLITE_PATH", BaseConfig.STORAGE_SQLITE_PATH),
        help="SQLite DB path for market OHLCV storage.",
    )
    parser.add_argument(
        "--exchange-id",
        default=getattr(BaseConfig, "MARKET_DATA_EXCHANGE", "binance"),
