...
DEFAULT_DB_PATH = "logs/lumina_quant.db"
...
def build_summary(df_equity, df_trades):
    ...
    if not df_trades.empty:
        ...
        closed = (
            df_trades[df_trades["realized_pnl"] != 0]
            if "realized_pnl" in df_trades.columns
            else pd.DataFrame()
        )
        if not closed.empty:
            out["win_rate"] = float((closed["realized_pnl"] > 0).mean())
            if "realized_return_pct" in closed.columns:
                out["avg_trade_return_pct"] = float(closed["realized_return_pct"].mean())
            out["best_trade_pnl"] = float(closed["realized_pnl"].max())
            out["worst_trade_pnl"] = float(closed["realized_pnl"].min())
...
with st.sidebar.expander("Optimization Search Space", expanded=False):
    default_optuna_json = json.dumps(OptimizationConfig.OPTUNA_CONFIG, indent=2, ensure_ascii=False)
    default_grid_json = json.dumps(OptimizationConfig.GRID_CONFIG, indent=2, ensure_ascii=False)
    ...
    optimize_trials = st.number_input(
        ...
        min_value=1,
        max_value=5000,
        value=int(OptimizationConfig.OPTUNA_CONFIG.get("n_trials", 20)),
        step=1,
    )
...
col5.metric("Realized PnL", f"{summary['realized_pnl']:.2f}")
    ...
    with st.expander("Metric Definitions", expanded=False):
        ...
        for metric_name, definition in METRIC_DEFINITIONS.items():
            metric_rows.append(
                {
                    "Metric": metric_name,
                    "Value": _format_metric_value(metric_name, performance.get(metric_name))
                    if performance
                    else "N/A",
...
with tab_exec:
    ...
    if not trade_analytics.empty:
        closed = trade_analytics[trade_analytics["realized_pnl"] != 0]
        if not closed.empty:
...
with tab_report:
    ...
    live_strategy_name = live_col_3.selectbox(
        "Live Strategy",
        ["RsiStrategy", "MovingAverageCrossStrategy"],
        index=0 if strategy_name == "RsiStrategy" else 1,
