...
from . import research_runner as _runner
...
def _report_candidates_from_stage2_results(
    *,
    ...
    return _runner._research_report_builder().report_candidates_from_stage2_results(
...
def _attach_cross_candidate_correlations(report_candidates: Sequence[dict[str, Any]]) -> None:
    _runner._research_report_builder().attach_cross_candidate_correlations(report_candidates)
...
def _candidate_research_report_payload(
    *,
    ...
    return {
        "schema_version": "v2",
        "generated_at": _runner.datetime.now(_runner.UTC).isoformat(),
...
def _run_candidate_research_with_adapted_candidates(
    *,
    ...
    stage2_results = _runner._select_stage2_results(
        adapted=adapted,
        cache=cache,
...
def run_candidate_research(
    *,
    ...
    min_bundle_bars: int = _runner._MIN_BARS,
) -> dict[str, Any]:
    """Evaluate candidate manifest into train/val/OOS report contract (v2)."""
    base_tf = _runner._normalize_candidate_research_base_timeframe(base_timeframe)
    market_data_settings = _runner._current_research_market_data_settings()
    scoring = _runner._resolve_research_run_scoring_config(
        score_config=score_config,
        stage1_keep_ratio=stage1_keep_ratio,
    )
    adapted = _runner._adapt_candidate_inputs(candidates, max_candidates=max_candidates)

    if not adapted:
        return _runner._empty_candidate_research_report(
            base_timeframe=base_tf,
            strategy_timeframes=strategy_timeframes,
            symbol_universe=symbol_universe,
            stage1_keep_ratio=stage1_keep_ratio,
...
def build_default_candidate_rows(
    *,
    ...
    rows = build_binance_futures_candidates(
        symbols=symbols or _runner._current_research_market_data_settings()["symbols"],
        timeframes=timeframes or _runner.CANONICAL_STRATEGY_TIMEFRAMES,
    )
    out = [_runner.adapt_legacy_candidate(item.to_dict()) for item in rows]
