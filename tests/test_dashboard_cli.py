...
def test_build_dashboard_command_uses_npm_for_next_mode() -> None:
    command = dashboard.build_dashboard_command(mode="next")
...
def test_dashboard_env_exposes_launch_mode_and_compatibility_path() -> None:
    contract = dashboard.build_dashboard_contract(mode="next", compat_path="/api/python/dashboard/overview")
...
def test_dashboard_main_run_uses_dashboard_web_cwd_for_next_mode(monkeypatch) -> None:
    captured: dict[str, object] = {}
    ...
    exit_code = dashboard.main(["--run", "--mode", "next"])
...
def test_dashboard_main_print_contract_emits_json(capsys) -> None:
    exit_code = dashboard.main(["--mode", "next", "--print-contract"])
